...
- **可审计** — `artifacts/evidence-log.jsonl` 记录每个文件的读取目的和提取的关键洞察（key_findings），读者可以验证研究是否遗漏关键文件、洞察是否有源可溯
...
## 为什么需要 evidence-log.jsonl

研究过程中会读取大量源码、配置、文档文件。从每个文件提取的洞察（"7 层认证链违反单一职责但换取原子性"）是研究的核心价值——这些洞察不能只存在对话上下文里。

**问题**：对话上下文会被压缩。如果洞察只存在上下文里，会话压缩或中断后，Report Agent 写报告时只能：
1. 重读所有文件（浪费，且代码可能已变）
2. 靠记忆补全（不可靠，容易虚构）
3. 跳过细节（报告质量下降）

**解决方案**：`artifacts/evidence-log.jsonl`（JSON Lines，append-only）。每读一个文件，立即追加一行，包含文件路径、读取目的、提取的关键洞察（至少 3 条）、证据强度。

**为什么是 JSONL 而非 JSON 数组**：
- append-only：不需要读-改-写整个文件，直接追加一行
- 每行独立：会话中断时已写入的行不会损坏
- 可流式读取：恢复时逐行解析，内存友好
- 可 grep：`grep "gateway" evidence-log.jsonl` 快速定位某文件的证据

**为什么 key_findings 是洞察而非摘要**：
- 摘要（"这个文件 1960 行实现了认证"）不增加理解，等于没读
- 洞察（"7 层认证链违反单一职责但换取请求处理的原子性"）是研究的产出
- 报告写作时直接引用洞察，而非重新从摘要推导
...
**为什么需要 replaces 失效机制**：
- evidence-log 是真正的 append-only（禁止修改已有行），但代码会变——同一个文件旧版本和新版本的洞察不能同时有效
- 如果 gateway.ts 从"7 层认证"变成"8 层认证"，不修改旧条目，而是追加新条目并设 `replaces: "ev-023"`
- Report Agent 读取时扫描所有 `replaces` 字段，计算被取代的 ID 集合，排除这些条目——避免新旧证据并存导致报告自相矛盾
- 这是真正的 append-only 与增量更新之间唯一的调和方式：不改旧内容，只追加新条目声明取代关系
- 失效粒度是 `(file, purpose)`——同一个文件不同研究角度的证据独立失效
