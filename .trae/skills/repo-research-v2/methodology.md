...
> 本文档记录 Repository Research 的方法论与设计理念。**给人看，不给模型加载。** 解释"为什么这样做"，而不是"怎么做"。
...
| **问题驱动** | 研究从提出架构问题开始，通过针对性收集证据不断修正知识模型。而非机械阅读整个仓库。 |

---

## 为什么 Repository Model 是第一产物

Karpathy LLM Wiki 的最大观点：**不要每次重新理解。**
...
如果对所有仓库套用同一套通用问题（"系统如何划分职责？数据如何流动？"），会产生正确但平庸的研究——每个仓库的报告都差不多，失去洞见价值。
...
- 证据收集围绕特定问题展开，而非机械阅读整个仓库
- 每个结论都源于回答某个问题
- 问题随理解演化而演化（见 question-framework.md）

问题不是一次性生成的清单，而是随理解演化的推理轨迹。
...
`user_input` 原样保存禁止转义——用户的原始掸辞可能包含关键上下文（如"重点关注安全"或"忽略前端"），任何转义都可能丢失这些信号。
...
这个设计解决了 LLM 常见的"确认偏误"问题：LLM 会回答一个问题就认为完成，却从未挑战自己的答案。

生命周期强制研究者先 `answered` 再 `validated`，才能进入报告阶段。
