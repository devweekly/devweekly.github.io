# Research Report: synthetic-agent-4zwh0M

## Executive Summary

synthetic-agent-4zwh0M is analyzed with deterministic pipeline. Detected signals: hasAgent, hasTool.
Findings: 16. README contradictions: 1.

## Top Claims

# Evidence Brief: synthetic-agent-4zwh0M
...
| F-009 | Q8 | high | 0.13 | 0.60 | ✅ verified | verified | README claims "LLM Integration" but code signals do not confirm it (hasLLM=false). Treated as docume |
...
| Dimension | Value |
|-----------|-------|
| Repository | synthetic-agent-4zwh0M (package: synthetic-agent) |
| Manifest | src/index.js (javascript) |
| Version | 1.0.0 |
| Source files | 6 |
| Top languages | .js (6), .md (1), .json (1), .txt (1) |
...
|--------|-------|----------------|
| Modules | 6 | — |
| Import edges | 3 | edge/node ratio: 0.50 |
| Import cycles | 0 | no cycles — clean layering |
| Functions | 0 | N/A |
| Classes | 4 | 0.7 classes/module |

**Coupling assessment**: edge/node ratio 0.50 → low

**Most depended-upon modules** (high in-degree = core/foundation):
  - `src.agent` (in-degree: 1)
  - `src.runner` (in-degree: 1)
  - `src.tool` (in-degree: 1)
  - `src.index` (in-degree: 0)
  - `src.memory` (in-degree: 0)
  - `tests.agent.test` (in-degree: 0)

**Most influential modules** (high PageRank = architectural bottleneck):
  - `src.tool` (PageRank: 0.3004)
  - `src.runner` (PageRank: 0.2425)
  - `src.agent` (PageRank: 0.1744)
  - `src.index` (PageRank: 0.0943)
  - `src.memory` (PageRank: 0.0943)
  - `tests.agent.test` (PageRank: 0.0943)
...
**Derived indicators**:
  - Coupling density: 0.50 edges/module
  - CI: none detected ⚠
...
| Artifact | 3 | Non-code artifact (test/eval/config/doc/dataset/evidence) |
...
| 3 | `src/agent.js` | 90 | high in-degree (+40); high PageRank (+50) |
| 4 | `src/memory.js` | 90 | high in-degree (+40); high PageRank (+50) |
| 5 | `src/runner.js` | 90 | high in-degree (+40); high PageRank (+50) |
| 6 | `src/tool.js` | 90 | high in-degree (+40); high PageRank (+50) |
| 7 | `tests/agent.test.js` | 60 | high in-degree (+40); high PageRank (+50); test file (-30) |
...
2. `src/agent.js` — high in-degree (+40); high PageRank (+50)
3. `src/memory.js` — high in-degree (+40); high PageRank (+50)
4. `src/runner.js` — high in-degree (+40); high PageRank (+50)
5. `src/tool.js` — high in-degree (+40); high PageRank (+50)
...
- **? H2-ai-agent** (medium): This is an AI-agent / LLM-related project with prompts and/or tools
