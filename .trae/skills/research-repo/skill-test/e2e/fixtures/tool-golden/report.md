# Research Report: synthetic-tool-o1MFVO

## Executive Summary

synthetic-tool-o1MFVO is analyzed with deterministic pipeline. Detected signals: hasPlugin.
Findings: 16. README contradictions: 1.

## Top Claims

# Evidence Brief: synthetic-tool-o1MFVO
...
| F-009 | Q8 | high | 0.13 | 0.60 | ✅ verified | verified | README claims "SQL" but code signals do not confirm it (hasSQL=false). Treated as documentation-only |
...
- **Finding**: README claims "SQL" but code signals do not confirm it (hasSQL=false). Treated as documentation-only claim until source evidence is found.
- **Importance**: high
...
- **Lifecycle**: verified (candidate→hypothesis→supported→verified→decision/reusable_pattern)
- **Support**:
  - [regex] README.md — README mentions "SQL" by=README
...
| Documentation | 100% | 13% (low) | 1 | 1 | Full coverage. |
...
| Dimension | Value |
|-----------|-------|
| Repository | synthetic-tool-o1MFVO (package: synthetic-tool) |
| Manifest | src/index.js (javascript) |
| Version | 1.0.0 |
| Source files | 6 |
| Top languages | .js (6), .md (1), .json (1) |
...
|--------|-------|----------------|
| Modules | 6 | — |
| Import edges | 1 | edge/node ratio: 0.17 |
| Import cycles | 0 | no cycles — clean layering |
| Functions | 0 | N/A |
| Classes | 4 | 0.7 classes/module |

**Coupling assessment**: edge/node ratio 0.17 → low

**Most depended-upon modules** (high in-degree = core/foundation):
  - `src.plugin` (in-degree: 1)
  - `src.driver` (in-degree: 0)
  - `src.extension` (in-degree: 0)
  - `src.index` (in-degree: 0)
  - `src.ui.view` (in-degree: 0)
  - `tests.plugin.test` (in-degree: 0)

**Most influential modules** (high PageRank = architectural bottleneck):
  - `src.plugin` (PageRank: 0.2701)
  - `src.driver` (PageRank: 0.1460)
  - `src.extension` (PageRank: 0.1460)
  - `src.index` (PageRank: 0.1460)
  - `src.ui.view` (PageRank: 0.1460)
  - `tests.plugin.test` (PageRank: 0.1460)
...
|---------|-----------|------------|-------------|----------------|------------------|
| Factory | 1 | 0.6 | ★★★☆☆ | low | ViewPart (factory methods: createControl) |

#### Factory (1 instances)

**Reusability (script heuristic — LLM should refine)**:
- **Applicability**: Found in: ui/
...
**Testing**: 1 test files, 1 test functions
  Test/source ratio: 0.17 → adequate coverage
...
**Derived indicators**:
  - Coupling density: 0.17 edges/module
  - CI: none detected ⚠
...
|---------|------|------|
| Entity | 4 | Code unit (function/class/agent/planner/runner) |
...
| 3 | `src/driver.js` | 90 | high in-degree (+40); high PageRank (+50) |
| 4 | `src/extension.js` | 90 | high in-degree (+40); high PageRank (+50) |
| 5 | `src/plugin.js` | 90 | high in-degree (+40); high PageRank (+50) |
| 6 | `src/ui/view.js` | 90 | high in-degree (+40); high PageRank (+50) |
| 7 | `tests/plugin.test.js` | 60 | high in-degree (+40); high PageRank (+50); test file (-30) |
...
2. `src/driver.js` — high in-degree (+40); high PageRank (+50)
3. `src/extension.js` — high in-degree (+40); high PageRank (+50)
4. `src/plugin.js` — high in-degree (+40); high PageRank (+50)
5. `src/ui/view.js` — high in-degree (+40); high PageRank (+50)
