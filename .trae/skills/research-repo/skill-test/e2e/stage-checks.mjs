...
export function validateAnalyzerStage(dir) {
  ...
  const requiredRoots = ["repository", "archetypeHints", "symbols", "architecture", "prompts", "tools", "tests", "evaluations"];
  for (const key of requiredRoots) {
    checks.push(checkStage(`has ${key}`, data[key] !== undefined, `Missing root key: ${key}`, stage));
  }

  checks.push(checkStage("repository has name", typeof data.repository?.name === "string", "Missing repository.name", stage));
  checks.push(checkStage("archetypeHints has archetype", typeof data.archetypeHints?.archetype === "string", "Missing archetypeHints.archetype", stage));
  checks.push(checkStage("symbols has modules", Array.isArray(data.symbols?.modules) || typeof data.symbols?.modules === "object", "Missing symbols.modules", stage));
...
export function validateEvidenceBriefStage(dir) {
  ...
  checks.push(checkStage("has Repository Identity", /(^|\n)#{1,3}\s*Repository\s+Identity/i.test(text), "Missing Repository Identity", stage));
  checks.push(checkStage("has Archetype Hints", /(^|\n)#{1,3}\s*Archetype\s+Hints/i.test(text), "Missing Archetype Hints", stage));
  checks.push(checkStage("has Key Evidence", /(^|\n)#{1,3}\s*Key\s+Evidence/i.test(text), "Missing Key Evidence", stage));
  checks.push(checkStage("has Design Decisions", /(^|\n)#{1,3}\s*Design\s+Decisions/i.test(text), "Missing Design Decisions", stage));
