...
**repository-model.json 唯一写入者**——从 evidence 合并/更新 Repository Knowledge Model。完成 **Phase 2（Architecture Reconstruction）+ Phase 3（Runtime Reconstruction）**。
...
`repository-model.json`（完整结构见 [model-schema.md §2](../model-schema.md)）：
...
```json
{
  ...
  "architecture": { /* §4 */ },
  "runtime": { /* §5 */ },
  "design_decisions": [ /* §6, Reasoning Agent 协作 */ ],
  "evolution": { /* §7, Reasoning Agent 协作 */ },
  "hypotheses": [ /* §8, Reasoning Agent 维护 */ ],
  "evidence": [ /* §9, 从 evidence-log 合并 */ ],
  "questions": [ /* §10, 从 questions/summary 合并 */ ],
  "coverage": { /* §11 */ }
}
```
...
- `evidence`：从 evidence-log.jsonl 合并（去重、按 tier 排序）
...
对每条新 evidence:
  ...
  2. 如果节点存在 → 合并 evidence（追加到 evidence[] 字段，重算 confidence）
