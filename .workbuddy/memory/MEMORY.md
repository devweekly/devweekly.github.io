...
配套 checklist（2026-09-13 共改五轮：结构修正 → 逐条去中性化 → 分层语义修正 → **正文剥离说明**；
现 **2758** 行 / 主表 **595** 项 + **18** Invariants + **4** Decision Gates + 附录 A 181 项 +
附录 B（框架·读法·评分·版本）+ 附录 C（62 节章节说明）。只有第三轮 +1 条 P00-21，其余各轮不增删条目）：
...
- **编号约定（重要）**：P01–P14 的条目**本版已重排为 `1`–`512`**（`1`–`336` 与上一版一致；`337` 起因 FSI Overlay
  ...
  **主表只数有 ID 的条目**，P00.6 Gate / P00.7 Artifact 这类无条目的说明小节不计入（第七轮起已整节移入附录 C）。
...
- **正文零说明（第七轮定型，绝对不要再往正文加说明）**：用户明确要求「正文就应该是 checklist，
  说明作为附录章节」「『本版 / 上一版』这类 changelog 不要污染正文」。因此：
  **第一、二章只允许出现 标题 / 表格 / 条目行 / 空行**，散文、`>` 引用、`> **层**：L1` 这类定位标注、
  ASCII 示意图全部搬进 **附录 C — 各章说明与原依据**（按正文章节顺序 C.1..C.62）；
  文档顶部只留 4 行导航。后续再加条目时，**不要**在正文里补说明段落。
- **条目必带方向标签 + 达标线（第七轮改写法，改条目时必须遵守）**：用户明确要求「不能中性提问，
  要能判断 yes / no 倾向性与做到什么程度」；随后又否决了早期的 `应为 / 应有`（原话：「不是正常的汉语表达，
  AI 味道刺鼻难闻」）。**现行写法**：每条问题末尾 `｜必须：… ｜达标线：…`，方向标签**只能**取
  `必须` / `禁止` / `条件`（写明条件，条件没写清不能判 N/A）/ `可选` 四选一；
  `达标线` 必须给可核对尺度（粒度 per Run·Agent·tenant·dataset + 覆盖 + 频率 + 举证位置）
  并补一句**「什么不算达标」**。共 776 处（主表 595 + 附录 A 181）。
  P00 五张问题表用**追加列**，表头 `必须 / 禁止 / 条件 / 可选 ｜达标线`；Invariants / Gates 表用追加列
  `判据 / 典型 Fail`（不变量是陈述句，不问方向）。读法定义见 **附录 B.13**，编号与元数据约定见 **附录 B.14**，
  B.2 有 `Expectation` 字段，B.3 说明它是 Result 判定的下限口径。
...
- **三个 Review Depth + L1 的两个 Stage（第三轮定型，别再改回「互斥分类」）**：
  ...
  正文不再标 depth / Stage（标注已随说明收进附录 C，B.11 的表负责导航）。
...
- **交付形态：正文零说明，条目与说明彻底分离**。骨架 = H1 + **4 行导航**（正文只放检查项；说明见附录 B / C）
  → `# 一、Checklist 主表（P00 + P01–P14）`（P01 降 H2、P01.1 降 H3；**只允许 标题 / 表格 / 条目行 / 空行**）
  → `# 二、Architecture Invariants 与 Decision Gates`
  → `# 附录 A — 补充控制项（A01..A181，不计入主表）`
  → `# 附录 B — 评审框架与说明`（B.1 框架·版本·分层结构 / B.2 记录字段+Depth·Stage·Requirement /
  B.3 评分方式 / B.4 总览 + 编号总账（**唯一权威数字来源**）/ B.5 P0 十项红线 + 准入前置 /
  B.6 6 个关键证明问题 / B.7 评分板 / B.8 与 AWS WAF 关系 /
  B.9 版本演进记录（389 → 594 → 595，changelog 只在这里出现）/ B.10 跨框架映射 /
  B.11 三个 Review Depth 与 L1 的两个 Stage / B.12 编号变更对照 /
  **B.13 条目阅读方式 / B.14 编号与元数据约定**）
  → `# 附录 C — 各章说明与原依据`（C.1..C.62，正文剥离出来的层·Stage 标注、Base·Delta 边界、
  框架依据、判断说明与 ASCII 示意图，按正文章节顺序）
  → `# 参考`
...
- 起草过程语与「你们已经决定 MCP…」「你们已经有 LangSmith…」这类对话残留**一律不进交付稿**
  （历史上分两轮清掉 15 处）；「对话稿转成稿」的可复用流程见 skill `skills/article-de-ai` 第 27 节
...
- 第十轮把**正文主体换掉**：正文 = A–L 十二节 **120 个架构评审问题**（`Q001`–`Q120`），
  原 431 项**一字未改**降为 **附录 D — Evidence Checks**（旧编号继续有效，历史引用不废）。
  正文用 `Q` 编号、证据层用旧编号，两个独立序列，跨层回查走 B.4 总览表与 B.12。
  正文的 depth / Stage 写在**节标题**里（`## A. … ｜L1 Decision · INIT`），不再用正文 `> **层**：` 标注；
  第一章仍然只允许 heading / 表格行 / 空行（第 8 轮定下的「正文零说明」规则）
