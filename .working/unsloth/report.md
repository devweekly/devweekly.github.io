...
> **如果只记住一件事**：Unsloth 不是新的训练框架，而是 Hugging Face 之上的透明加速层——靠 import-time 补丁与手写融合内核，让现有 notebook 不改一行就提速约 2x、省显存 70%；代价是与上游强耦合、双后端分支脆弱、Apple Silicon 仅能跑 SFT。
...
**本章回答**：Unsloth 是什么、它解决什么业务问题、整体架构（high-level）什么样。
**核心结论**：它是架在 Hugging Face 之上的「加速桥」，不是训练框架，也不是模型托管服务。
...
**本章回答**：架构风格、塑造所有决策的硬约束、架构中心与演进方向。
**核心结论**：三种风格（装饰 / 门面 / 注册）叠加，全部服务于同一目标——用户零改动。
...
**本章回答**：5 个最关键设计决策各自的意图、机制、约束与权衡。
**核心结论**：import-time patch 是总闸门，其余决策都依赖它在用户无感下生效。
...
**本章回答**：系统边界为何存在、新能力如何扩展。
**核心结论**：边界是约束的产物，扩展靠注册而非代码分叉。
...
**本章回答**：权重与优化器状态如何被重新组织以降低显存。
**核心结论**：量化（压权重）+ GaLore（压优化器状态）是两个正交杠杆，叠加得到 70% 省显存。
...
**本章回答**：一次微调请求的完整路径、Studio 桌面链路、过载如何降级与隔离。
**核心结论**：优化发生在训练开始之前（import/patch/quantize），而非训练之中。
...
**本章回答**：基于证据（而非空泛评价）的架构优势。
**核心结论**：透明零改动 + 内核融合真收益 + 双后端统一 API 是 adoption 三支柱。
...
**本章回答**：最具架构影响的风险及其触发后果。
**核心结论**：与上游版本强耦合是最高频破碎源，且常静默降级而非报错。
...
**本章回答**：改哪里危险、哪里安全（Blast Radius）。
**核心结论**：动后端闸门与升上游大版本最危险；加模型族最安全。
...
**本章回答**：可跨项目复用的工程哲学。
**核心结论**：装饰器优于框架替换、注册优于分叉、稳定核心 + 可变外围。
...
同一概念只在此处详细展开一次，其余章节引用而非重复，以降低认知负荷、保持主线聚焦。

| 概念 | 首次定义 | 其他出现 |
| --- | --- | --- |
| import-time monkey-patch | §3.3 / §4.1（d-1） | §1, §2.1, §5.1, §8 S1, §11.1 |
| 双后端门面（_IS_MLX） | §3.1 / §4.3（d-3） | §2.2, §5.1, §7.1, §9 R4 |
| QuantType / 量化身份 | §4.5（d-5）/ §6.1 | §5.2, §6.3, §10 |
| MODEL_REGISTRY 注册扩展 | §3.1 / §5.2 | §6.1, §11.2 |
| MLX capability-subset（SFT-only） | §5.1 / §4.3 | §9 R4, §10 |
| Studio native boundary | §5.1 / §7.2 | §2.2, §4.4 |
| 双显存杠杆（量化 + GaLore） | §6.2 | §4.5, §11.4 |
